...
TIPS <- function(simulated_data) {
  ...
  # Define M-step and EM functions for CoMM
  M_step_comm <- function(old, w1, y, w2, z, n1, n2) {
    ...
    alphaest <- as.numeric(solve(t(mu_ui_i) %*% t(w2) %*% w2 %*% mu_ui_i + sum(diag(w2 %*% sigma_ui_i %*% t(w2)))) %*% t(z) %*% w2 %*% mu_ui_i)
    E1 <- t(y) %*% y - 2 * t(y) %*% w1 %*% mu_ui_i + t(mu_ui_i) %*% t(w1) %*% w1 %*% mu_ui_i + sum(diag(t(w1) %*% w1 %*% sigma_ui_i))
    E2 <- t(z) %*% z - 2 * alphaest * t(z) %*% w2 %*% mu_ui_i + alphaest^2 * t(mu_ui_i) %*% t(w2) %*% w2 %*% mu_ui_i + alphaest^2 * sum(diag(w2 %*% sigma_ui_i %*% t(w2)))
    E3 <- t(mu_ui_i) %*% mu_ui_i + sum(diag(sigma_ui_i))
    sigma1est <- 1 / n1 * E1
    sigma2est <- 1 / n2 * E2
    sigmauest <- 1 / 5 * E3
  ...
  for (i in 1:m1) {
    ...
    log_likelihood_alt <- loglik(c(mle_comm[i, 1], mle_comm[i, 2], mle_comm[i, 3], mle_comm[i, 4]), 
                                 w1 = w1, y = ynew, w2 = w2, z = znew, n1 = n1, n2 = n2)
    # Null hypothesis: alpha_i is set to 0
    log_likelihood_null <- loglik(c(mle_comm[i, 1], mle_comm[i, 2], mle_comm[i, 3], 0), 
                                  w1 = w1, y = ynew, w2 = w2, z = znew, n1 = n1, n2 = n2)
  ...
  # Average estimates across genes
  mle_comm_avg <- c(colMeans(mle_comm[, 1:3]), mle_comm[, 4])
  
  # --- PrediXcan Elastic Net ---
  ...
  for (i in 1:m1) {
    ...
    cv_fit <- cv.glmnet(w1_i, y_i, alpha = 0.5, family = "gaussian")
    best_lambda <- cv_fit$lambda.min
    fit <- glmnet(w1_i, y_i, alpha = 0.5, lambda = best_lambda)
    # Store coefficients
    coefficients_en[[i]] <- coef(fit)
    # Predict expression in testing data
    predicted_expression <- predict(fit, newx = w2_i)
    # Association test
    association_result <- lm(z_i ~ predicted_expression)
    if (dim(summary(association_result)$coefficients)[1] > 1) {
      p_values_EN[i] <- summary(association_result)$coefficients[2, "Pr(>|t|)"]
    } else {
      p_values_EN[i] <- NA
    }
  }
  ...
  for (i in 1:m1) {
    ...
    z_i <- truealpha[i] * w2_i %*% u_i + e2
    # Fit LASSO model
    cv_fit <- cv.glmnet(w1_i, y_i, alpha = 1, family = "gaussian")
    best_lambda <- cv_fit$lambda.min
    fit <- glmnet(w1_i, y_i, alpha = 1, lambda = best_lambda)
    # Store coefficients
    coefficients_lasso[[i]] <- coef(fit)
    # Predict expression in testing data
    predicted_expression <- predict(fit, newx = w2_i)
    # Association test
    association_result <- lm(z_i ~ predicted_expression)
    if (dim(summary(association_result)$coefficients)[1] > 1) {
      p_values_lasso[i] <- summary(association_result)$coefficients[2, "Pr(>|t|)"]
    } else {
      p_values_lasso[i] <- NA
    }
  }
  ...
  # Estimate SNP effects on gene expression
  u_i_est <- matrix(0, nrow = m1, ncol = p1)
  for (i in 1:m1) {
    y_i <- y[, i]
    # Use ridge regression (alpha = 0) for estimation
    cv_fit <- cv.glmnet(wg1, y_i, alpha = 0, family = "gaussian")
    best_lambda <- cv_fit$lambda.min
    fit <- glmnet(wg1, y_i, alpha = 0, lambda = best_lambda)
    # Store coefficients excluding intercept
    u_i_est[i, ] <- as.vector(coef(fit))[-1]
  }
  
  # Predict gene expressions in wg2
  predictor <- wg2 %*% t(u_i_est)
  model <- lm(z ~ predictor + 0)  # Exclude intercept
  # Extract p-values
  ...
  result_list <- list(
    ...
    mse_l1 = mse_l1,
    mse_l2 = mse_l2,
    mle_comm = mle_comm_avg,
    p_values_comm = p_values_lrt,
    p_values_EN = p_values_EN,
    p_values_lasso = p_values_lasso,
